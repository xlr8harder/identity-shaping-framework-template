...
|---------|-------------|---------------|
| `backend` | Training backend | `tinker` or `unsloth` |
...
`backend: tinker` and `backend: unsloth` are integrated today. With Tinker, your
`base_model` must be a model that Tinker can train. List available models with:
...
For local Unsloth SFT, use a HuggingFace/Unsloth causal language model that
fits your GPU. `axolotl` and `prime` are reserved names so those integrations
can use the same `isf train run` command when they land. See
[Backend Selection](../backend-selection.md) and
...
Backend-specific options are documented in
[Backend Functionality](../backend-functionality.md). Keep common knobs such as
`epochs`, `batch_size`, `lora_rank`, `max_length`, and `learning_rate` at the top
level; put backend-specific knobs under `backend_options`.
...
Training saves checkpoints at configured intervals. Tinker checkpoints can be
sampled directly. Unsloth writes local adapter and optional merged-model
...
After Tinker training completes, the checkpoint is automatically registered. For
Unsloth, serve the adapter or merged model behind an OpenAI-compatible local
server and register the served model name. You can manually rebuild the
registry:
